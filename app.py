...
def gerar_contrato_aqua_sem_rt_pdf(dados: dict) -> bytes:
    ...
    servicos = _v("servicos", "Orientação técnica operacional, inspeção visual, recomendações de rotina e acompanhamento documental sem emissão de RT/ART.")
    ...
    def _header_footer(canvas, doc_obj):
        ...
        canvas.drawRightString(w - 1.45 * cm, h - 0.99 * cm, "Prestação de serviços operacionais/consultivos para piscinas")
    ...
    story.append(_raw("CONTRATO DE PRESTAÇÃO DE SERVIÇOS<br/>AQUA GESTÃO — SEM RESPONSABILIDADE TÉCNICA", "AquaTituloSemRT"))
    story.append(_raw("Documento comercial para atendimento sem RT/ART mensal. Não substitui contrato de Responsabilidade Técnica.", "AquaSubSemRT"))
    ...
    linhas_id = [
        ...
        [
            ...
            _raw(f"<b>{_html.escape(nome)}</b><br/>{doc_label}: {_html.escape(documento)}<br/>Endereço: {_html.escape(endereco)}<br/>Responsável: {_html.escape(responsavel)}<br/>Telefone: {_html.escape(telefone)}", "AquaTableSemRT"),
        ],
    ]
    ...
    h("1. Objeto")
    p(f"A CONTRATADA prestará à CONTRATANTE serviços comerciais e operacionais relacionados a piscinas, incluindo: {servicos}")
...
with st.expander("🧾 Contrato Aqua Gestão sem RT / sem ART — Pessoa física ou jurídica", expanded=False):
    ...
    with _aqs2:
        aq_sem_doc = st.text_input(
            ...
            value=st.session_state.get("cnpj_condominio", ""),
        ...
        aq_sem_tel = st.text_input(
            ...
            value=st.session_state.get("whatsapp_cliente", "") or st.session_state.get("telefone_cliente", ""),
        )
    ...
    aq_sem_servicos = st.text_area(
        ...
        height=80,
        value="Acompanhamento operacional, inspeção visual, orientação de rotina, recomendações de tratamento, registro documental e apoio técnico sem emissão de RT/ART.",
    )
